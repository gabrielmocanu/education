...
void reverseList(Nod **head, Nod **stack){

  Nod *aux = *head;

  while(aux != NULL){
    addFirst(stack, aux -> val);
    aux = aux -> next;
  }

  aux = *head;
  while(aux != NULL){
    aux -> val = deleteFirstStack(stack);
    aux = aux -> next;
  }
}
...
int main(){
  ...
  reverseList(&linkedList, &stack);
  printLinkedList(linkedList);

  reverseListInPlace(&linkedList);
  printLinkedList(linkedList);
